...
<?= $this->section('content'); ?>		
	<div style="margin:auto;max-width:400px"
      ...
      style="border:1px solid white;border-top:3px solid rgba(255,0,215,107);border-radius:5px;border-bottom:3px solid rgba(255,0,215,107);border-left:5px solid white;border-right:5px solid white">
			<form id="form-signin" method="post" action="<?= base_url('/signup');?>">
        <?= csrf_field();?>
        <div class="form-group">
          <label for="exampleInputUsername1">Username</label>
          <input type="text" name="username" class="form-control" id="exampleInputUsername1" aria-describedby="usernameHelp" placeholder="Masukan Username" required>    
        </div>
			  <div class="form-group">
			  ...
			  <button type="submit" class="btn btn-primary"
			  	id="button-signin"
			  	style="background:rgba(255,0,215,107);border:0px">Daftar</button>
			</form>

			<div class="mt-4">
				<a href="<?= base_url('/signin');?>">Sudah Punya Akun</a>
			</div>
		</div>
	</div>
<?= $this->endSection();?>
